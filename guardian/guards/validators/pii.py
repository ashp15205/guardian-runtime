...
# ---------------------------------------------------------------------------
# UPI detection: candidate regex + suffix allowlist gate
# The candidate captures the suffix in group(1) for the gate check.
# This pattern is NOT in PII_PATTERNS — used only inside detect().
# ---------------------------------------------------------------------------
UPI_CANDIDATE_PATTERN: re.Pattern = re.compile(
    r"\b[\w.\-]+@([a-z]{2,})\b", re.IGNORECASE
)

# Suffix allowlist — gate runs BEFORE a PIIMatch is created, not as post-filter.
# Adding a new suffix here is all that's needed to support new UPI handles.
...
class PIIDetector:
    ...
    def detect(self, text: str) -> list[PIIMatch]:
        ...
        for pii_type in self.enabled_types:
            if pii_type == PIIType.UPI_ID:
                # UPI uses separate candidate + gate logic
                matches.extend(self._detect_upi(text))
                continue
    ...
    def _detect_upi(self, text: str) -> list[PIIMatch]:
        """
        UPI detection with suffix gate.

        Step 1: Find all @-mentions with UPI_CANDIDATE_PATTERN
        Step 2: Read suffix from group(1)
        Step 3: Gate — if suffix not in KNOWN_UPI_SUFFIXES, discard immediately
        Step 4: Only then create a PIIMatch

        user@gmail.com → suffix="gmail" → not in set → discarded ✅
        user@ybl       → suffix="ybl"   → in set     → PIIMatch  ✅
        """
        matches: list[PIIMatch] = []
        for m in UPI_CANDIDATE_PATTERN.finditer(text):
            suffix = m.group(1).lower()
            if suffix not in KNOWN_UPI_SUFFIXES:
                continue  # Gate: discard before any PIIMatch is created
            matches.append(
                PIIMatch(
                    pii_type=PIIType.UPI_ID,
                    matched_text=m.group(),
                    start=m.start(),
                    end=m.end(),
                    confidence=0.95,
