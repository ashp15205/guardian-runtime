"""CLI command: guardian dashboard — open local observability dashboard."""
...
def dashboard_command(port: int, host: str, no_browser: bool):
    ...
    click.echo("\n")
    click.echo("  ⛨  Guardian Dashboard")
    click.echo(f"  ─────────────────────────────────────────")
    click.echo(f"  URL          : {url}")
    click.echo(f"  Data source  : ~/.guardian/logs/")
    ...
    from guardian.dashboard.server import create_dashboard_app
