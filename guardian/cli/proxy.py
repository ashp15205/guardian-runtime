"""CLI command: guardian proxy — start the local HTTP proxy server."""
...
@click.option(
    ...
    show_default=True,
    help="Path to the Guardian policy YAML file.",
)
@click.option(
    "--reload",
    is_flag=True,
...
def proxy_command(port: int, host: str, policy: str, reload: bool):
    """
    Start the Guardian local proxy server.
    ...
    if not policy_path.exists():
        ...
        click.echo("   Create one with: guardian validate --help", err=True)
        sys.exit(1)

    click.echo("\n")
    click.echo("  ⛨  Guardian Runtime Proxy")
    click.echo(f"  ─────────────────────────────────────────")
    click.echo(f"  Listening on : http://{host}:{port}")
    click.echo(f"  Policy       : {policy}")
    click.echo(f"  Dashboard    : guardian dashboard (run in another terminal)")
    click.echo(f"")
    ...
    from guardian.proxy.server import create_proxy_app
    ...
    uvicorn.run(
        ...
        host=host,
        port=port,
        log_level="warning",  # suppress verbose uvicorn logs; Guardian logs what matters
        reload=reload,
    )
