...
| **Phase 1 — Core SDK** | 3–6 | Jun 16 – Jul 13 | Guards, YAML engine, local storage, PyPI publish | ✅ `pip install guardian-runtime` works |
...
>
> Your main pitch is: *"Are you afraid of sending your LLM logs to a cloud observability tool? We built a local-first governance layer. The data never leaves your machine."* Pitch this on Reddit, Twitter, and Pune meetups.
...
- Build secret/credential detector (OpenAI `sk-`, AWS `AKIA`, GitHub `ghp_`, Stripe, Razorpay, Groq, Slack, HuggingFace + generic KEY=value patterns). Two confidence tiers: HIGH (0.95, block) and MEDIUM (0.70, flag).
- Build YAML parser using Pydantic models.
- Write 40+ unit tests for accurate detection and redaction without cloud dependencies.
